...
- (nonnull instancetype)initWithNamespace:(nonnull NSString *)ns
    ...
    if ((self = [super init])) {
        ...
        _ioQueue = dispatch_queue_create("Alexander.Cache", DISPATCH_QUEUE_SERIAL);
        ...
        if (directory != nil) {
            ...
            NSString *path = [[[self userCacheDirectory] stringByAppendingPathComponent:@"Alexander.Cache"] stringByAppendingPathComponent:ns];
            _diskCachePath = path;
        }
...
- (void)migrateDiskCacheDirectory {
    if ([self.diskCache isKindOfClass:[SDDiskCache class]]) {
        ...
        dispatch_once(&onceToken, ^{
            // ~/Library/Caches/Alexander.Cache/default/
            NSString *newDefaultPath = [[[self userCacheDirectory] stringByAppendingPathComponent:@"Alexander.Cache"] stringByAppendingPathComponent:@"default"];
